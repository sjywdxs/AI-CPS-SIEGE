function interval = BinarySplit(reward, lb, ub, minDist, minNum, maxDist)
...
%   interval: lb and up of valid interval
        ...
        if ubNew-lb >= maxDist
            interval = [lb, lb+maxDist];
        elseif ubNew-lb > minDist && numValidState > minNum
            interval = BinarySplit(reward, lb, ubNew, minDist, minNum,maxDist);
        else
            interval = [lb, ub];
        end
